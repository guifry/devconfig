...
require("lazy").setup({
	...
	-- LSP Plugins
	{
		...
		config = function()
			...
			local servers = {
				-- Python: basedpyright + ruff (per https://www.lazyvim.org/extras/lang/python)
				-- basedpyright: type checking, hover, go-to-def, auto-import code actions
				-- ruff: fast linting/formatting (hover disabled to avoid overlap)
				basedpyright = {
					settings = {
						basedpyright = {
	...
	-- Auto-import for Python (workaround for neovim#31655 - basedpyright code actions bug)
	{
		...
		keys = {
			{ "<leader>ci", function() require("lspimport").import() end, desc = "Import symbol under cursor" },
		},
	},

	-- Python venv selector (like VSCode's interpreter picker)
	{
...
do
	...
	local function detect_filetype(lines)
		...
		if trimmed:match("^<!DOCTYPE") or trimmed:match("^<html") then
			return "html"
		end
		...
		if
			trimmed:upper():match("^SELECT%s")
			or trimmed:upper():match("^INSERT%s")
			or trimmed:upper():match("^UPDATE%s")
			or trimmed:upper():match("^DELETE%s")
			or trimmed:upper():match("^CREATE%s")
			or trimmed:upper():match("^ALTER%s")
			or trimmed:upper():match("^WITH%s")
		then
			return "sql"
		end
