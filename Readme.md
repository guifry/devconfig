...
Generated by `scripts/ssh-setup.sh`. `~/.ssh/config`:
```
...
in the agent and GitHub authenticates you as whichever it accepts first — silently
the wrong account.

### The host alias picks the account

There is no identity to "switch". The host alias in a remote URL selects the key,
so each repo self-selects. Get the URL right once and it stays right:

```bash
git clone git@github.com-gforey-ext:Kpler/<repo>.git   # work
git clone git@github.com-guifry:guifry/<repo>.git      # personal
```

Prefer the explicit alias over plain `github.com` — the plain host resolves to the
work key here, so a personal clone through it authenticates as `gforey-ext`.

Fix a repo pushing as the wrong account:
```bash
git remote set-url origin git@github.com-gforey-ext:Kpler/<repo>.git
```

### Two independent layers

| Layer | Used for | Identity chosen by |
|-------|----------|--------------------|
| SSH | `git clone/fetch/push/pull` | host alias in the remote URL |
| `gh` CLI | PRs, issues, API | active `gh` account (`gh auth switch`) |

They do not talk to each other. Changing the `gh` account does not change who
`git push` authenticates as. For a Kpler PR you usually need both: the right remote
alias *and* `gh auth switch -u gforey-ext`.

Commit author identity is a third, separate thing — handled automatically by the
per-directory includes, see `gitconfig-identity.example`.
